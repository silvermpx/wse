...
enum InboundEvent {
    Connect { conn_id: String, cookies: String },
    Message { conn_id: String, value: serde_json::Value },
    RawText { conn_id: String, text: String },
    Binary { conn_id: String, data: Vec<u8> },
...
fn normalize_json_keys(mut val: serde_json::Value) -> serde_json::Value {
    if let serde_json::Value::Object(ref mut map) = val {
        ...
        if !map.contains_key("p") {
            if let Some(v) = map.remove("payload") {
...
fn message_category(msg_type: &str) -> &'static str {
    ...
    match msg_type {
        "server_ready" | "server_hello" | "client_hello_ack"
        | "connection_state_change" | "subscription_update"
        | "snapshot_complete" | "error" | "pong" | "PONG" => "WSE",
        _ => "U",
    }
}
                            ...
                            if let Some(t_val) = val.get("t").and_then(|t| t.as_str()) {
                                if t_val == "ping" {
                                    ...
                                    let pong = format!(
                                        ...
                                        timestamp, server_ts, server_ts.saturating_sub(timestamp).max(0)
                                    );
...
impl RustWSEServer {
    ...
    fn send(&self, conn_id: &str, data: &str) -> PyResult<()> {
        let tx = self.cmd_tx.as_ref().ok_or_else(|| PyRuntimeError::new_err("Not running"))?;
    ...
    fn send_event(
        ...
        let cmd_tx = self.cmd_tx.as_ref().ok_or_else(|| PyRuntimeError::new_err("Not running"))?;

        // #7: Dedup — skip if this event ID was already sent
        if let Ok(Some(id_val)) = event.get_item("id") {
            if let Ok(id_str) = id_val.extract::<String>() {
        ...
        // #8: Per-connection rate limiter (token bucket)
        {
            ...
            let state = rates.entry(conn_id.to_owned()).or_insert_with(|| PerConnRate {
        ...
        if let Ok(Some(t_val)) = event.get_item("t") {
            if let Ok(s) = t_val.extract::<String>() {
                msg_type = s.clone();
        ...
        if !map.contains_key("id") {
            map.insert("id".to_string(), serde_json::Value::String(Uuid::now_v7().to_string()));
        }
        ...
        if compression_threshold > 0 && payload_bytes.len() > compression_threshold {
            ...
            cmd_tx.send(ServerCommand::SendPrebuilt {
                conn_id: conn_id.to_owned(),
    ...
    fn broadcast(&self, data: &str) -> PyResult<()> {
        let tx = self.cmd_tx.as_ref().ok_or_else(|| PyRuntimeError::new_err("Not running"))?;
        tx.send(ServerCommand::BroadcastText { data: data.to_owned() })
            .map_err(|_| PyRuntimeError::new_err("Channel closed"))?;
        Ok(())
    }
    ...
    fn broadcast_bytes(&self, data: &[u8]) -> PyResult<()> {
        let tx = self.cmd_tx.as_ref().ok_or_else(|| PyRuntimeError::new_err("Not running"))?;
        tx.send(ServerCommand::BroadcastBytes { data: data.to_vec() })
            .map_err(|_| PyRuntimeError::new_err("Channel closed"))?;
        Ok(())
    }
    ...
    fn get_connection_count(&self) -> PyResult<usize> {
        ...
        tx.send(ServerCommand::GetConnectionCount { reply: reply_tx })
            .map_err(|_| PyRuntimeError::new_err("Channel closed"))?;
        reply_rx.blocking_recv().map_err(|_| PyRuntimeError::new_err("Reply dropped"))
    }
    ...
    fn disconnect(&self, conn_id: &str) -> PyResult<()> {
        let tx = self.cmd_tx.as_ref().ok_or_else(|| PyRuntimeError::new_err("Not running"))?;
        tx.send(ServerCommand::Disconnect { conn_id: conn_id.to_owned() })
            .map_err(|_| PyRuntimeError::new_err("Channel closed"))?;
        Ok(())
    }
    ...
    fn drain_inbound(
        ...
        let events: Vec<InboundEvent> = py.detach(|| {
            ...
            if guard.is_empty() {
                ...
                let (new_guard, _) = self.shared.inbound_condvar
